...
_HTML_TAGS = {
    'b': _ENT_BOLD, 'strong': _ENT_BOLD,
    'i': _ENT_ITALIC, 'em': _ENT_ITALIC,
    'u': _ENT_UNDERLINE, 'ins': _ENT_UNDERLINE,
    's': _ENT_STRIKETHROUGH, 'strike': _ENT_STRIKETHROUGH, 'del': _ENT_STRIKETHROUGH,
    'code': _ENT_CODE,
    'pre': _ENT_PRE,
    'spoiler': _ENT_SPOILER,
}
...
def _html_to_entities(text:str)->tuple[str, list[tuple[int,int,int,str|None]]]:
    ...
    while i < n:
        ...
        if ch == '<':
            ...
            while j < n and (text[j].isalpha() or text[j].isdigit()):
                j += 1
            tag = text[name_start:j].lower()
            if not tag:
                while j < n and text[j] != '>': j += 1
                if j < n: j += 1
                result.append(text[i:j]); i = j; continue
            url = None
            if not is_close and tag == 'a':
                while j < n and text[j] != '>':
                    if text[j:j+5].lower() == 'href=':
                        j += 5
                        if j < n and text[j] in ('"', "'"):
                            q, j = text[j], j + 1
                            hs = j
                            while j < n and text[j] != q: j += 1
                            url = text[hs:j]
                            if j < n: j += 1
                        else:
                            hs = j
                            while j < n and text[j] not in (' ', '>'): j += 1
                            url = text[hs:j]
                    else:
                        j += 1
            while j < n and text[j] != '>': j += 1
            ...
            if is_close:
                ...
                matched = False
                while stack:
                    s, otag, ourl = stack.pop()
                    if otag == tag:
                        length = written - s
                        if length > 0:
                            if otag in _HTML_TAGS:
                                entities.append((s, length, _HTML_TAGS[otag], None))
                            elif otag == 'a' and ourl:
                                entities.append((s, length, _ENT_TEXT_URL, ourl))
                        matched = True
                        break
                    tmp.append((s, otag, ourl))
                for item in reversed(tmp): stack.append(item)
                if not matched:
                    result.append(text[i:j])
            else:
                if tag in _HTML_TAGS or tag == 'a':
                    stack.append((written, tag, url))
                else:
                    result.append(text[i:j])
            i = j
        else:
    ...
    written = len(cleaned)
    while stack:
        s, otag, url = stack.pop()
        length = written - s
        if length > 0:
            if otag in _HTML_TAGS:
                entities.append((s, length, _HTML_TAGS[otag], None))
            elif otag == 'a' and url:
                entities.append((s, length, _ENT_TEXT_URL, url))
    return cleaned, entities


def _md_tokenize(text:str, fmt:str):
    i, n = 0, len(text)
    buf: list[str] = []
    while i < n:
        ch = text[i]
        ...
        if fmt == 'v2' and ch == '*' and i + 1 < n and text[i + 1] == '*':
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'BOLD'); i += 2; continue
        if fmt == 'legacy' and ch == '*':
            if i + 1 < n and text[i + 1] == '*':
                buf.append('*'); buf.append('*'); i += 2; continue
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'BOLD'); i += 1; continue
        if fmt == 'v2' and ch == '_' and i + 1 < n and text[i + 1] == '_':
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'ITALIC'); i += 2; continue
        if ch == '_' and not (i + 1 < n and text[i + 1] == '_'):
            prev_is_underscore = fmt == 'legacy' and buf and buf[-1] == '_'
            if not prev_is_underscore:
                if buf: yield ('TEXT', ''.join(buf)); buf.clear()
                yield ('FMT', 'ITALIC'); i += 1; continue
        if fmt == 'v2' and ch == '~' and i + 1 < n and text[i + 1] == '~':
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'STRIKE'); i += 2; continue
        if fmt == 'legacy' and ch == '~':
            if i + 1 < n and text[i + 1] == '~':
                buf.append('~'); buf.append('~'); i += 2; continue
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'STRIKE'); i += 1; continue
        if fmt == 'v2' and ch == '|' and i + 1 < n and text[i + 1] == '|':
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'SPOILER'); i += 2; continue
        if ch == '`' and i + 2 < n and text[i + 1] == '`' and text[i + 2] == '`':
            ...
            if i > ls: lang = text[ls:i]
            yield ('PRE', lang); continue
        if ch == '`':
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            yield ('FMT', 'CODE'); i += 1; continue
        if ch == '[':
            if buf: yield ('TEXT', ''.join(buf)); buf.clear()
            i += 1
            lt_buf: list[str] = []
            ...
            while i < n and depth:
                if text[i] == '\\' and i + 1 < n:
                    lt_buf.append(text[i + 1]); i += 2
                elif text[i] == '[':
                    lt_buf.append('['); depth += 1; i += 1
                elif text[i] == ']':
                    depth -= 1
                    if depth: lt_buf.append(']')
                    i += 1
                else:
                    lt_buf.append(text[i]); i += 1
            lt = ''.join(lt_buf)
            if i < n and text[i] == '(':
                i += 1
                us = i
                ...
                u = text[us:i]
                if i < n: i += 1
                yield ('LINK', lt, u); continue
            buf.append('['); buf.append(lt)
...
_MD_ENT = {'BOLD': _ENT_BOLD, 'ITALIC': _ENT_ITALIC, 'STRIKE': _ENT_STRIKETHROUGH,
           'SPOILER': _ENT_SPOILER, 'CODE': _ENT_CODE}

def _md_to_entities(tokens) -> tuple[str, list[tuple[int,int,int,str|None]]]:
    entities: list[tuple[int,int,int,str|None]] = []
    result: list[str] = []
    ...
    for tok in tokens:
        ...
        if tp == 'TEXT':
            ...
            if stack and stack[-1][1] == kind:
                s, _ = stack.pop()
                length = len(''.join(result)) - s
                if length > 0:
                    entities.append((s, length, _MD_ENT[kind], None))
            else:
                stack.append((len(''.join(result)), kind))
        elif tp == 'PRE':
            if stack and stack[-1][1] == 'PRE':
                s, _ = stack.pop()
                length = len(''.join(result)) - s
                if length > 0:
                    entities.append((s, length, _ENT_PRE, None))
            else:
                stack.append((len(''.join(result)), 'PRE'))
        elif tp == 'LINK':
            _, lt, url = tok
            s = len(''.join(result))
            result.append(lt)
            entities.append((s, len(lt), _ENT_TEXT_URL, url))
    cleaned = ''.join(result)
    written = len(cleaned)
    while stack:
        s, kind = stack.pop()
        length = written - s
        if length > 0 and kind != 'PRE':
            entities.append((s, length, _MD_ENT.get(kind, _ENT_PRE), None))
        elif length > 0 and kind == 'PRE':
            entities.append((s, length, _ENT_PRE, None))
    return cleaned, entities

def _markdownv2_to_entities(text:str)->tuple[str, list[tuple[int,int,int,str|None]]]:
    return _md_to_entities(_md_tokenize(text, 'v2'))

def _markdown_to_entities(text:str)->tuple[str, list[tuple[int,int,int,str|None]]]:
    return _md_to_entities(_md_tokenize(text, 'legacy'))

_PARSE_MODES = {
    'HTML': _html_to_entities,
    'MarkdownV2': _markdownv2_to_entities,
    'Markdown': _markdown_to_entities,
}

def _text_to_entities(text:str, parse_mode:str)->tuple[str, list[tuple[int,int,int,str|None]]]:
    parser = _PARSE_MODES.get(parse_mode)
    if parser is None:
        raise ValueError(f"Unknown parse_mode: {parse_mode}. Supported: {list(_PARSE_MODES)}")
    return parser(text)

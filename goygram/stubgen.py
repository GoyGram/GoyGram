...
def _core_pyi(client_py: Path) -> list[str]:
    ...
    lines: list[str] = []
    seen: set[str] = set()
    classes = [node for node in tree.body if isinstance(node, ast.ClassDef) and node.name in {"AppCore", "GoyGram"}]
    classes.sort(key=lambda node: node.name != "GoyGram")
    for node in classes:
        for item in node.body:
            ...
            if item.name in seen:
                continue
            seen.add(item.name)
            if any(isinstance(value, ast.Name) and value.id == "property" for value in item.decorator_list):
                lines.append("    @property")
            end = item.body[0].lineno - 1 if item.body else item.end_lineno or item.lineno
            header = rows[item.lineno - 1:end]
            if not header:
                continue
            header[-1] = header[-1].rstrip()
            if header[-1].endswith(":"):
                result = " -> Any" if item.returns is None else ""
                header[-1] = header[-1][:-1] + result + ": ..."
            lines.extend(header)
    return lines
...
def generate(out_dir: Path) -> Path:
    ...
    rpc: list[str] = [
        ...
        "from . import telegram as _types",
        "from .filters import Filter",
        "from .types.obj import Obj",
        "",
    ...
    for name, spec in sorted(methods.items()):
        ...
        for alias in _aliases(name):
            if alias in used_fn:
                continue
            used_fn.add(alias)
            rpc.append(f"    async def {alias}(self, **kwargs: Unpack[_types.{ident}]) -> Any: ...")
    ...
    for name, spec in sorted(methods.items()):
        ...
        rpc.append("    @overload")
        rpc.append(f"    async def mt_req(self, act: Literal[{lits}], /, **kwargs: Unpack[_types.{ident}]) -> Any: ...")
    rpc.append("    async def mt_req(self, act: str, /, **kwargs: Any) -> Any: ...")
    for name, _fields in bot_methods:
        ident = _ident(name)
        lits = ", ".join(f'"{x}"' for x in dict.fromkeys([name, ident, *_aliases(name)]))
        rpc.append("    @overload")
        rpc.append(f"    async def bot_req(self, act: Literal[{lits}], /, **kwargs: Unpack[_types.{ident}]) -> Any: ...")
